...
for (const file of files) {
  ...
  console.log(`Inserting: ${title.slice(0, 40)}...`);
  const [note] = await api('POST', '/rest/v1/notes', {
    title, content: body, summary: meta.summary, tags: meta.tags,
    category_id: catId, author_id: adminId, is_private: false
  });
  console.log(`  OK (${note.content.length} chars)`);
}

console.log('\nDone! All notes imported with proper newlines.');
